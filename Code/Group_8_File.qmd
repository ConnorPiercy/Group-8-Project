...
3. Values close to the detection limit: Values reported in square brackets [ ] are identified by the authors as being close to the LOD. Some of these values are reported as [0]. Since \(\log_{10}(0)\) is undefined, values recorded as zero are replaced with a small positive pseudo-count of 0.1 before applying the \(\log_{10}\) transformation. This allows these observations to remain in the analysis rather than removing the corresponding samples.
...
Need to answer these main results questions: - Can you assume that the data follow a multivariate normal distribution? - Are there “surprising” observations? - (Yes found in Mahalanobis Distnace plot 5 of them)
...
5.  normality check for assuming normality questions (Chi-square QQ plot on mahalonbis distances)
